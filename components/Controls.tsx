...
import {
  Settings, Layout, Type, Palette, Printer, Plus, Trash2, Sparkles, AlertCircle, LogOut, Image as ImageIcon, Layers, Calculator, Eye, Save, FolderOpen, Download, MessageSquare, Globe, Search, Eraser, LayoutGrid, ChevronRight, Tags, BookOpen, History, Clock, Package
} from 'lucide-react';
...
export const Controls: React.FC<ControlsProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'data' | 'layout' | 'design' | 'history'>('data');
      ...
      <div className="flex border-b border-gray-200 bg-white">
        {[
          ...
          <button
            ...
            className={`flex-1 py-3.5 text-[10px] font-black uppercase tracking-widest flex flex-col items-center gap-1 transition-all border-b-2 relative ${activeTab === tab.id ? 'text-indigo-600 border-indigo-600 bg-indigo-50/50' : 'text-slate-400 border-transparent hover:text-slate-600 hover:bg-slate-50'
              }`}
          >
            <tab.icon className={`w-4 h-4 ${activeTab === tab.id ? 'text-indigo-600' : 'text-slate-400'}`} />
            {tab.label}
            {'badge' in tab && tab.badge && (
              <span className="absolute top-2 right-2 bg-indigo-500 text-white text-[8px] font-black rounded-full w-4 h-4 flex items-center justify-center">
